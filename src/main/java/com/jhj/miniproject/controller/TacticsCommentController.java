...
@RestController
@RequestMapping("/api/tactics/comment")
public class TacticsCommentController {
	...
	public ResponseEntity<?> writeComment(
		...
		comment.setTacticsBoard(_tacticsBoard.get());
	...
	public ResponseEntity<?> loadComment(@PathVariable("boardId") Long boardId) {
		...
		Optional<TacticsBoard> _tacticsBoard = tacticsBoardService.getTacticsBoard(boardId);
		if(_tacticsBoard.isEmpty()) {
			return ResponseEntity.status(404).body("게시글이 존재하지 않습니다.");
		}
		...
		List<Comment> comments = commentService.getCommentListByTacticsBoard(tacticsBoard);
